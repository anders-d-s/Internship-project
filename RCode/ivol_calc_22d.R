...
for (factor in factors) {
  ...
  #data is from first non NA of the factor with 12 months of data (assuming 22 trading days a year)
  start_index <- which(!is.na(daily_factors[[factor]]))[1]
  length <- length((start_index + 21):nrow(daily_factors))
  cat("Factor:", factor, "\n")
  
  for (i in 1:length) {
    
    df <- daily_factors[(start_index + i - 1):(start_index + 21 + i - 1),c("date",factor,"mkt", "mkt_lagged")]
    
    fit <- lm(as.formula(paste(factor, "~ mkt + mkt_lagged")), data = df)
    
    ivol_df[(start_index + 21 + i - 1),factor] <- sqrt(var(fit[["residuals"]]))
    
  }
}

save(ivol_df, file = "Data/daily_ivol_22d.RData")
